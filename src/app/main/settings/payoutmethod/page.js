...
let ATYPES = ["E-Wallet", "Bank", "Remittance", "Cheque"]
...
export default function PayoutMethod(props) {
    ...
    const init = async ()=>{
        ...
        try{           
            ...
            // }, 2000);                  
            const ret =  await callApi("/setting/pm/", "GET") 
            if (ret.status==200){                
                setdata(ret.data)
                setloadstate("success")
            }else{    
    ...
    const handleCloseAE = ()=>{
        ...
        if (bEdit){
            ...
            toast.success('New payout method successfully saved!')
        }
    ...
    const handleSuccessDelete = ()=>{
        setselectedData(tmpForm)
        setshowDelete(false)
        toast.success('Payout method sucessfully deleted!')
        init()
    }
