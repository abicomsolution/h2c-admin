...
function ConfirmDelete(props) {
    ...
    const {showConfirm, setshowConfirm, onYes, selectedProduct} = props      
    ...
    const handleConfirm= async ()=>{               
        try{
            ...
            setdeleteState("delete")
            const ret =  await callApi("/product/delete/"+selectedProduct._id,  "GET") 
            if (ret.status==200){                
                setdeleteState("success")
                onYes()
            }else{         
    ...
    return(
        <Modal show={showConfirm}  onClose={handleClose} className={`${interFont.className}`} >            
            <ModalBody>              
                ...
                <p className='text-xl mt-4 px-5'>Are you sure you want to delete this product?</p>                
                <p className='text-lg font-semibold mt-4 px-5'>{selectedProduct?.productname}</p>                
